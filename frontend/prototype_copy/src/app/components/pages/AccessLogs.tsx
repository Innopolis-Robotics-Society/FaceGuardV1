...
import {
  ...
  X, CheckCircle, XCircle, AlertTriangle, Eye, DoorOpen, Clock, Filter, Loader,
} from "lucide-react";
import { toast } from "sonner";
import { apiClient } from "../../../api/client";
import { transformEventToLog } from "../../../utils/transformers";
import type { Event } from "../../api/types";
import type { LogEntry } from "../../../utils/transformers";
...
export function AccessLogs() {
  ...
  async function loadLogs() {
    try {
      setLoading(true);
      const events = await apiClient.getEvents(100);
      const transformed = events.map(transformEventToLog);
      setLogs(transformed);
    } catch (error) {
  ...
  function exportCSV() {
    const rows = filtered
      .map((l) => `${l.id},${l.name},${l.date},${l.time},${l.confidence.toFixed(1)}%,${l.status},${l.action}`)
      .join("\n");
    ...
    <div className="space-y-5">
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20 rounded-2xl" style={CARD}>
          ...
          <p className="text-sm font-medium text-white mb-1">Loading logs...</p>
        </div>
      ) : paginated.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 rounded-2xl" style={CARD}>
