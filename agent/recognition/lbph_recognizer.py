...
class LBPHRecognizer(BaseRecognizer):
    ...
    def __init__(self):
        ...
        if Config.MODEL_FILE.exists() and Config.LABELS_FILE.exists():
            try:
                ...
                logger.warning(f"Failed to load existing LBPH model: {e}")
    ...
    def _load_face_detector(self):
        ...
        logger.info("LBPH face detector loaded successfully")
        return detector
    ...
    def train_model(self) -> Dict[str, Any]:
        ...
        """
        logger.info("Starting LBPH model training...")
        ...
        logger.info(f"Training LBPH with {total_photos} photos from {people_count} people")
        ...
        logger.info(f"LBPH model trained and saved: {people_count} people, {total_photos} photos")
    ...
    def load_model(self):
        ...
        self.is_trained = True
        logger.info(f"LBPH model loaded successfully: {len(self.label_map)} people")
    ...
    def recognize_face(self, frame: np.ndarray) -> Optional[Dict[str, Any]]:
        ...
        if not self.is_trained or self.recognizer is None:
            logger.warning("LBPH model not trained, cannot recognize")
            return None
        ...
        # Filter false positives: check aspect ratio (faces are roughly square)
        aspect_ratio = w / h
        if aspect_ratio < 0.7 or aspect_ratio > 1.4:
            logger.debug(f"Rejected detection: bad aspect ratio {aspect_ratio:.2f}")
            return None

        # Filter false positives: check if face is too small or too large
        face_area = w * h
        frame_area = gray.shape[0] * gray.shape[1]
        area_ratio = face_area / frame_area

        if area_ratio < 0.02:  # Too small (less than 2% of frame)
            logger.debug(f"Rejected detection: too small (area ratio: {area_ratio:.4f})")
            return None

        if area_ratio > 0.8:  # Too large (more than 80% of frame)
            logger.debug(f"Rejected detection: too large (area ratio: {area_ratio:.4f})")
            return None
        ...
        # LBPH returns distance: 0 = perfect match, higher = worse
        # Formula: confidence% = 100 - (distance / max_distance * 100)
        # We use threshold * 2 as max_distance to get better scaling
        max_distance = Config.RECOGNITION_THRESHOLD * 2
        confidence_percent = max(0, min(100, 100 - (confidence / max_distance * 100)))
        ...
        return {
            ...
            "raw_distance": float(confidence),  # Original distance value
