...
- Распознаёт лица по LBPH distance: значение ниже `RECOGNITION_THRESHOLD`
...
# "Person recognized: {person_id} (distance: 45.2)"
...
- В логах есть события "recognized" с пограничной или высокой LBPH distance
...
- В логах события "unknown" с LBPH distance около текущего threshold
...
OpenCV LBPH возвращает raw distance, а не вероятность. Ниже distance означает
более сильное совпадение; equality с threshold не принимается как match.
